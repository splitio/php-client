...
use SplitIO\Http\Psr7\Request;
use SplitIO\Http\Psr7\Uri;

class HttpSocketAdapter implements HttpAdapterInterface
{
    private $host;
    private $port;
    private $path;
    ...
    protected $user_agent = 'SplitIO HttpClient v0.1';
    ...
    public function doRequest(RequestInterface $request, $timeout = 30)
    {
        $uri = $request->getUri();

        $this->host = $uri->getHost();
        $this->port = ($uri->getPort() !== null)? $uri->getPort() : 80;
        $this->path = (!empty($uri->getPath()))? $uri->getPath() : '/';

        if (!$fp = fsockopen($this->host, $this->port, $errno, $errstr, $timeout)) {
        ...
        fwrite($fp, $this->buildRequest($request));
    ...
    protected function buildRequest(RequestInterface $r)
    {
        $path = (!empty($r->getUri()->getPath()))? $r->getUri()->getPath() : '/';

        $headers = array();
        $headers[] = "{$r->getMethod()} {$path} HTTP/1.0"; // Using 1.1 leads to all manner of problems, such as "chunked" encoding
        $headers[] = "Host: {$r->getUri()->getHost()}";
        $headers[] = "User-Agent: split-php-sdk";
    ...
    protected function __doRequest()
    {
